 ...
 public:
  ...
  Texture(const std::string& _type = "diffuse",
      ...
      : level(0),
        ...
        type(_type),
        unit(0),
        imgType(_imgType) {  // Do nothing here
    path = aiString(_path);
  }

  /* Load image and bind texture */
  Texture(const char* image_path) {  // Constructor
    reload(image_path);
  }
  ...
  void reload(const char* image_path,
    ...
    if (imgType > TEXTURE_JPG) {  // Convert surface format to enable transparency loading
    ...
    // ----------------------------------------------------------------------------------------
    if (imgType > TEXTURE_JPG) {  // If the texture supports transparency
      glTexImage2D(GL_TEXTURE_2D, level, GL_RGBA,
                   width, height, border,
                   GL_RGBA, GL_UNSIGNED_BYTE, surface->pixels);
    } else {  // Set RGB mode if transparency is not enabled
