...
//render each particle as a quad
static const GLfloat particle_quad[] = {
...
struct Particle {
	...
	//float size;  // edge length of the quad
	//float cameradistance; // *Squared* distance to the camera. if dead : -1.0f, needed???
...
public:
	ParticleSystem(const Shader& _shader, const Texture& _texture,
		const GLuint& _num_particles, const GLuint& _num_to_generate,
		const glm::vec3& _position_generator, const GLfloat& _range_x, const GLfloat& _range_z);
	
	void update(const GLfloat& dt);
	
	void draw(Camera camera, GLuint texture_unit);

	void setGeneratorPos(const glm::vec3& pos) {
		position_generator = pos;
	}

private:
	std::vector<Particle> particles;  //particle vector
	GLfloat* particle_position_data;  //positions of particle centers every frame (updated every frame)
	GLuint num_particles;  //max number of particles
	GLuint num_to_generate;  //number of new particles generated for a second
	GLuint num_particles_live; //number of living particles 

	glm::vec3 position_generator;  //the position of the generator
	GLfloat range_x, range_z;  //the range within which we respawn particles
	
	//GLfloat min_life, max_life; 

	Shader shader;  //a specific shader for rendering particles
	Texture texture;  //the texture we use to attach to every particle
	GLuint VAO, VBO_quad, VBO_particle_position;

	void init();  //deal with VAO, VBO, buffers binding, etc. 
	GLuint firstUnusedParticle();  //find first unused(dead) particle 
	void respawnParticle(Particle &particle);  //respawn a certain number of particles from (almost) a plane
};
*/
