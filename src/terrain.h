...
class Terrain {
public:
	static float size;  //the length of the terrain square
	static float maxHeight;  //just for normalizing  
private:
	float x, z;  //the left-bottom corner of the terrain, unnecessary

	int cells;  //the number of cells on each edge (the same in each edge)
	float** heights;  //the heights at each grid point
	...
	GLuint VAO, VBO_vert, VBO_normal, VBO_uv, EBO;

	void GenerateModel(const char* heightMap);
	void CalculateHeights(unsigned char* data);
	void SmoothHeights(float alpha);
	glm::vec3 CalculateNormal(const int& x, const int& y);
	float GetHeight(unsigned char* data, const int& x, const int& y);

public:
	Terrain(int gridX, int gridZ, Texture* texture, const char* heightMap);
	~Terrain();

	float GetTerrainHeight(const float& worldX, const float& worldZ);  //can be used later

	glm::mat4 getModel2World() { return model2world; }

	bool saveNormalMap(const std::string& filename);
...
//calculate the y coordinate when given x and z 
//p1, p2 and p3 form a triangle
//(x, y, z) is a point in this triangle
...
private:
	...
	void smoothingNormals()
	{
		...
		{ // Traversal -> all pixels
			for (GLint wloop = 0; wloop < cells; wloop++)
			{
				...
				glm::vec3 normal_right = (wloop < cells - 1) ?
					normals[_cur_cell + 1] : glm::vec3(0.0f, 0.0f, 0.0f);
				glm::vec3 normal_toprt = (hloop > 0 && wloop < cells - 1) ?
					normals[_cur_cell - cells + 1] : glm::vec3(0.0f, 0.0f, 0.0f);
				glm::vec3 normal_top = (hloop > 0) ?
					normals[_cur_cell - cells] : glm::vec3(0.0f, 0.0f, 0.0f);
				glm::vec3 normal_toplt = (hloop > 0 && wloop > 0) ?
					normals[_cur_cell - cells - 1] : glm::vec3(0.0f, 0.0f, 0.0f);
				glm::vec3 normal_left = (wloop > 0) ?
					normals[_cur_cell - 1] : glm::vec3(0.0f, 0.0f, 0.0f);
				glm::vec3 normal_btmlt = (hloop < cells - 1 && wloop > 0) ?
					normals[_cur_cell + cells - 1] : glm::vec3(0.0f, 0.0f, 0.0f);
				glm::vec3 normal_btm = (hloop < cells - 1) ?
					normals[_cur_cell + cells] : glm::vec3(0.0f, 0.0f, 0.0f);
				glm::vec3 normal_btmrt = (hloop < cells - 1 && wloop < cells - 1) ?
					normals[_cur_cell + cells + 1] : glm::vec3(0.0f, 0.0f, 0.0f);

				glm::vec3 _temp_vec = glm::normalize(normal_right + normal_left
					+ normal_top + normal_btm
					+ normal_toprt + normal_btmlt
					+ normal_toplt + normal_btmrt);
				// Compute the new normal vector
				normals[_cur_cell] = glm::normalize(normal_smooth * _temp_vec
					+ (1 - normal_smooth) * normals[_cur_cell]);
			}
		}
	}
	...
	void smoothing(GLfloat alpha)
	{
		...
		{ // Traversal -> all pixels
			...
			{ // Determine whether the pixel is on border
				...
				GLfloat _height_right = (wloop < cells - 1) ?
					heights[hloop][wloop + 1] : 0.0f;
				GLfloat _height_toprt = (hloop > 0 && wloop < cells - 1) ?
					heights[hloop - 1][wloop + 1] : 0.0f;
				GLfloat _height_top = (hloop > 0) ?
					heights[hloop - 1][wloop] : 0.0f;
				GLfloat _height_toplt = (hloop > 0 && wloop > 0) ?
					heights[hloop - 1][wloop - 1] : 0.0f;
				GLfloat _height_left = (wloop > 0) ?
					heights[hloop][wloop - 1] : 0.0f;
				GLfloat _height_btmlt = (hloop < cells - 1 && wloop > 0) ?
					heights[hloop + 1][wloop - 1] : 0.0f;
				GLfloat _height_btm = (hloop < cells - 1) ?
					heights[hloop + 1][wloop] : 0.0f;
				GLfloat _height_btmrt = (hloop < cells - 1 && wloop < cells - 1) ?
					heights[hloop + 1][wloop + 1] : 0.0f;
	...
	** Computes the normal vectors with given vertex position */
	glm::vec3 computeNormals(GLint hpos, GLint wpos)
	{
		...
		// ------------------------------------------------------------------------------------------
		glm::vec3 right_vec = (wpos < cells - 1) ?
			glm::vec3(cell_size, heights[hpos][wpos + 1] - heights[hpos][wpos], 0.0f)
			: glm::vec3(0.0f, 0.0f, 0.0f);
		glm::vec3 top_vec = (hpos > 1) ?
			glm::vec3(0.0f, heights[hpos - 1][wpos] - heights[hpos][wpos], -cell_size)
			: glm::vec3(0.0f, 0.0f, 0.0f);
		glm::vec3 toplt_vec = (hpos > 1 && wpos > 1) ?
			glm::vec3(-cell_size, heights[hpos - 1][wpos - 1] - heights[hpos][wpos], -cell_size)
			: glm::vec3(0.0f, 0.0f, 0.0f);
		glm::vec3 left_vec = (wpos > 1) ?
			glm::vec3(-cell_size, heights[hpos][wpos - 1] - heights[hpos][wpos], 0.0f)
			: glm::vec3(0.0f, 0.0f, 0.0f);
		glm::vec3 btm_vec = (hpos < cells - 1) ?
			glm::vec3(0.0f, heights[hpos + 1][wpos] - heights[hpos][wpos], cell_size)
			: glm::vec3(0.0f, 0.0f, 0.0f);
		glm::vec3 btmrt_vec = (hpos < cells - 1 && wpos < cells - 1) ?
			glm::vec3(cell_size, heights[hpos + 1][wpos + 1] - heights[hpos][wpos], cell_size)
			: glm::vec3(0.0f, 0.0f, 0.0f);
	...
	** The function to save normal map */
	...
	{
		if (normals.size() != cells * cells)
		{ // Check the normal vector size
