...
//		stacks(_stacks)
//	{
//		std::cout << "constructor of Ball" << std::endl;
//		std::cout << "radius is " << radius << std::endl;
//		std::cout << "number of slices is " << slices << std::endl;
//		std::cout << "number of stacks is " << stacks << std::endl;
//		genVertCord();
//		genUVCord();
//	}
//
...
//		// Bind buffers first
//		// bindBuffer();
//		/*std::cout << "setup of ball " << std::endl;*/
...
//
//		// draw this ball
//		//std::cout << "draw() of ball" << std::endl;
//		glBindVertexArray(VAO);
...
//	{
//		//std::cout << "draw with shader of ball " << std::endl;
//		// Install the shader
//		shader.install();
//		draw();
//		shader.uninstall();
...
//	** Note that vertex coordinates are also normal coordinates */
//	void genVertCord()
//	{
...
//		}
//
//		//std::cout << "generate vertex data " << std::endl;
//		//std::cout << "the first vertex is " << "[" << vertex[0] 
//		//	<<","<< vertex[1] << "," << vertex[2] << "]" << std::endl;
//		//std::cout << "the second vertex is " << "[" << vertex[0+3]
//		//	<< "," << vertex[1+3] << "," << vertex[2+3] << "]" << std::endl;
//		//std::cout << "the size of vertex data is " << vertex.size() << endl;
//	}
//
//	/* PRIVATE MEMBER:
...
//			}
//		}
//		//std::cout << "generate uv coordinates " << std::endl;
//		//std::cout << "the first uv coordinates are " << "[" 
//		//	<< uvcord[0] << "," << uvcord[1] << "]" << std::endl;
//		//std::cout << "the size of uv data is " << uvcord.size() << std::endl;
//	}
//
//
...
//
//	/* Default constructor & Constructor*/
//	Barriers(GLfloat _baseline = 0.75f,
...
{
public:
	Object(const GLfloat& _kd = 1.0f, const GLfloat& _ks = 0.0f, const GLfloat& _shininess = 10.0f) :kd(_kd), ks(_ks), shininess(_shininess) 
	{ 
		if (kd + ks > 1.0) {
...
public:
	...
	void setup()
	{
		...
		//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);  //cannot uncomment this line!
...
{
public:
	Prism(const GLuint& _n = 3, const GLfloat& _kd = 1.0, const GLfloat& _ks = 0.0, const GLfloat& _shininess = 1.0) 
		:Object(_kd, _ks, _shininess), n(_n) 
	{ 
		genVertexData();
	}
	void setup()
	{
		GLuint vert_VBO, normal_VBO, uv_VBO;
		// Generate vertex arrays and buffers
		glGenVertexArrays(1, &VAO);
		glGenBuffers(1, &vert_VBO);
		glGenBuffers(1, &normal_VBO);
...
private:
	void genVertexData()
	{
		vertices.reserve(6 * (n + 1));
		normals.reserve(6 * (n + 1));
		texCoords.reserve(4 * (n + 1));
		for (int i = 0; i < (n + 1); i++) {
			GLfloat x = cos(2 * M_PI * (GLfloat)i / n);
			GLfloat y = -0.5;
			GLfloat z = sin(2 * M_PI * (GLfloat)i / n);
			vertices.push_back(x); normals.push_back(x); texCoords.push_back((GLfloat)i / n);
			vertices.push_back(y); normals.push_back(0); texCoords.push_back(0);
			vertices.push_back(z); normals.push_back(z);
			y += 1;
			vertices.push_back(x); normals.push_back(x); texCoords.push_back((GLfloat)i / n);
			vertices.push_back(y); normals.push_back(0); texCoords.push_back(1);
			vertices.push_back(z); normals.push_back(z);
		}
	}
	
	GLuint n; //an n-sided polygonal base
